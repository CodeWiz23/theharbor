...
function openSettingsModal(type) {
    ...
    if (type === 'name') {
        ...
        currentUser.reauthenticateWithCredential(credential).then(() => currentUser.updatePassword(newPwd))
        ...
        if (lang && ['en','es','fr'].includes(lang.toLowerCase())) {
            ...
            db.collection('users').doc(currentUser.uid).update({ language: l }).then(() => {
                currentUserData.language = l;
                localStorage.setItem('harbor_language', l);
                alert('✅ Language set to ' + ({en:'English',es:'Español',fr:'Français'}[l]) + '!');
            }).catch(err => alert('❌ ' + err.message));
...
function viewGoldHistory() {
    ...
    db.collection('goldTransactions').where('fromUid', '==', currentUser.uid).orderBy('createdAt', 'desc').limit(10).get()
        .then(snap => {
            ...
            else snap.forEach(doc => { const d=doc.data(); msg += '• '+d.amount+'🪙 to '+(d.toName||'Someone')+' — '+(d.createdAt?d.createdAt.toDate().toLocaleDateString():'Recently')+'\n'; });
            alert(msg);
        }).catch(() => alert('💰 Gold Summary\n\nBalance: '+(currentUserData.goldBalance||0)+' 🪙'));
}
