    ...
    if (!currentUser) {
        container.innerHTML = `
            <div class="empty-state" style="padding:50px 20px;background:#fef3c7;border-radius:16px;border-left:4px solid #d97706;">
                ...
                <p style="margin-top:12px;font-size:0.9rem;color:#718096;">🌊 The Harbor is a safe community for sharing, healing, and growth.</p>
            </div>
        `;
        return;
    }
    ...
    query.get()
        .then((snapshot) => {
            if (snapshot.empty) {
                container.innerHTML = `
                    <div class="empty-state">
                        ...
                        ${canPostInCategory(currentCategory) ? `<a href="submit.html" class="btn btn-primary" style="display:inline-block;text-decoration:none;margin-top:12px;">📝 Share Your Story</a>` : ''}
                    </div>
                `;
                return;
            }
            ...
            snapshot.forEach((doc) => {
                ...
                story.id = doc.id;
                // Security rules handle visibility, but double-check client-side
...
function renderStories() {
    ...
    if (pageStories.length === 0) {
        container.innerHTML = `<div class="empty-state"><div class="big-emoji">🌊</div><h3>No stories found</h3><p>Be the first to share your story!</p></div>`;
    } else {
...
function renderStoryCard(story) {
    ...
    return `
        <div class="story-card" data-story-id="${story.id}">
            ...
            ${story.tags && story.tags.length ? `<div class="story-tags">${story.tags.map(tag => `<span class="story-tag">#${tag}</span>`).join('')}</div>` : ''}
            <div class="story-actions">
                ${reactionButtons}
                <button class="btn-action btn-edit" onclick="openCommentSection('${story.id}')">💬 ${story.commentCount || 0}</button>
                ...
                ` : ''}
                ${currentUser ? `<button class="btn-action btn-report" onclick="reportStory('${story.id}')">🚩 Report</button>` : ''}
            </div>
        </div>
    `;
}
...
function searchStories() {
    if (!currentUser) { alert('⚠️ Please log in to search stories.'); return; }
    currentPage = 1;
    applyFilters();
}
...
function openEditModal(storyId) {
    ...
    if (!story) return;
    if (story.userId !== currentUser.uid) { alert('You do not have permission to edit this story.'); return; }
...
function saveEdit() {
    ...
    if (!title || !content) { editError.textContent = '⚠️ Title and content are required.'; return; }
    if (title.length < 3) { editError.textContent = '⚠️ Title must be at least 3 characters.'; return; }
    if (content.length < 10) { editError.textContent = '⚠️ Content must be at least 10 characters.'; return; }
    db.collection('stories').doc(currentEditId).update({
        title: title,
        text: content,
        updatedAt: firebase.firestore.FieldValue.serverTimestamp()
    }).then(() => { closeEditModal(); loadStories(); alert('✅ Story updated successfully!'); })
...
function deleteStory(storyId) {
    ...
    const story = allStories.find(s => s.id === storyId);
    if (!story) return;
    if (story.userId !== currentUser.uid) { alert('You do not have permission to delete this story.'); return; }
    ...
    db.collection('stories').doc(storyId).delete()
        .then(() => { loadStories(); alert('✅ Story deleted successfully.'); })
...
function toggleVisibility(storyId) {
    ...
    const story = allStories.find(s => s.id === storyId);
    if (!story) return;
    if (story.userId !== currentUser.uid) { alert('You do not have permission to change visibility.'); return; }
    const newVisibility = story.visibility === 'public' ? 'private' : 'public';
    db.collection('stories').doc(storyId).update({ visibility: newVisibility })
        .then(() => { loadStories(); alert(`✅ Story is now ${newVisibility === 'public' ? 'Public' : 'Private'}.`); })
        .catch((error) => { alert('❌ Error updating visibility: ' + error.message); });
}
...
function addReaction(storyId, emoji) {
    ...
    const rect = document.getElementById('storiesContainer')?.getBoundingClientRect() || document.getElementById('storyCard')?.getBoundingClientRect();
    ...
    db.runTransaction((transaction) => {
        ...
        if (btn) {
            if (hasReacted) {
                ...
                if (!btn.querySelector('.checkmark')) btn.innerHTML += ' <span class="checkmark">✅</span>';
            }
        }
...
function loadAllUserReactions() {
    ...
    return db.collection('users').doc(currentUser.uid).collection('reactions').get()
        .then((snapshot) => {
            userReactions = {};
            snapshot.forEach((doc) => { userReactions[doc.id] = doc.data().emojis || []; });
            return userReactions;
        })
        .catch((err) => { console.error('Error loading user reactions:', err); userReactions = {}; return userReactions; });
}
...
(function addAnimationStyles() {
    if (!document.getElementById('animationStyles')) {
        ...
        style.textContent = `
            @keyframes floatUp { 0% { opacity:1; transform:translateY(0) scale(0.5) rotate(0deg); } 50% { opacity:1; transform:translateY(-150px) scale(1.3) rotate(20deg); } 100% { opacity:0; transform:translateY(-350px) scale(1) rotate(-10deg); } }
            @keyframes fireworkBurst { 0% { transform:translate(0,0) scale(0.5); opacity:1; } 100% { transform:translate(var(--tx), var(--ty)) scale(0); opacity:0; } }
            @keyframes sparkleBurst { 0% { transform:scale(0) rotate(0deg); opacity:1; } 100% { transform:scale(1.5) rotate(720deg); opacity:0; } }
            .floating-emoji { position:fixed; font-size:3rem; pointer-events:none; z-index:9999; animation:floatUp 1.5s ease-out forwards; }
            .firework-particle { position:fixed; pointer-events:none; z-index:9998; animation:fireworkBurst 0.8s ease-out forwards; }
            .sparkle-particle { position:fixed; pointer-events:none; z-index:9997; animation:sparkleBurst 1s ease-out forwards; }
        `;
...
function updateEmergencyBanner() {
    ...
    banner.innerHTML = `🆘 <strong>Emergency Number for ${country || 'your country'}:</strong> <a href="tel:${emergency}" style="color: #f5d6b3; font-weight: bold; text-decoration: underline;">${emergency}</a>${country ? `(${country})` : ''}`;
...
function openModal(mode) {
    ...
    ['authEmail', 'authPassword', 'authName', 'authFavorites', 'authCountry'].forEach(id => {
...
async function checkUsernameOnType() {
    ...
    statusDiv.innerHTML = available ? `<span style="color:#16a34a;">✅ Username is available!</span>` : `<span style="color:#dc2626;">❌ Username is taken. Please choose another.</span>`;
}
...
function handleAuth() {
    ...
    error.textContent = '';
    if (success) success.textContent = '';
    if (!email || !email.includes('@') || !email.includes('.')) { error.textContent = 'Please enter a valid email address.'; return; }
    ...
    if (!isLogin) {
        ...
        if (strength.score < 30) { error.textContent = '⚠️ Password is too weak. Please choose a stronger password.'; return; }
    }
    ...
    if (isLogin) {
        auth.signInWithEmailAndPassword(email, password)
            .then((userCredential) => {
                ...
                if (!user.emailVerified) {
                    error.innerHTML = `⚠️ Please verify your email first. Check your inbox (and spam folder)!<br><button onclick="resendVerification()" style="background:none;border:none;color:#1a4a4a;font-weight:600;cursor:pointer;text-decoration:underline;margin-top:4px;">🔄 Resend verification email</button>`;
                    auth.signOut();
        ...
        if (!name || name.length < 2) { error.textContent = 'Please enter a username (minimum 2 characters).'; submitBtn.disabled = false; submitBtn.textContent = '🚀 Create Account'; return; }
        ...
        if (!termsCheckbox || !termsCheckbox.checked) { error.textContent = '⚠️ Please agree to the Terms of Service and Privacy Policy.'; submitBtn.disabled = false; submitBtn.textContent = '🚀 Create Account'; return; }
        checkUsernameAvailability(name)
            .then((available) => {
                if (!available) { error.textContent = '❌ Username is already taken. Please choose another.'; submitBtn.disabled = false; submitBtn.textContent = '🚀 Create Account'; return Promise.reject('Username taken'); }
            ...
            .then((user) => {
                return db.collection('users').doc(user.uid).set({
                    name: name, email: email, gender: gender, favorites: favorites || 'Not specified',
                    country: country, emergencyNumber: '911', emailVerified: false, isAdmin: false,
                    isPublic: true, goldBalance: 10, goldReceived: 0, goldGiven: 0, followers: [],
                    following: [], storyCount: 0, commentCount: 0, likesReceived: 0,
                    language: 'en', theme: 'light', avatar: '👤', border: 'default',
                    createdAt: firebase.firestore.FieldValue.serverTimestamp(),
            ...
            .catch((err) => {
                if (err !== 'Username taken') { error.textContent = err.message; }
                submitBtn.disabled = false;
                submitBtn.textContent = '🚀 Create Account';
            });
    }
                ...
                if (doc.exists) {
                    ...
                    loadAllUserReactions().then(() => {
                        ...
                        if (document.getElementById('storiesContainer')) {
                            ...
                            currentCategory = cat;
                            if (!canSeeCategory(cat)) { currentCategory = 'all'; const url = new URL(window.location); url.searchParams.set('cat', 'all'); window.history.pushState({}, '', url); }
                            document.querySelectorAll('.tab').forEach((tab) => {
                        ...
                        if (window.location.pathname.includes('profile.html')) loadProfile();
                        if (window.location.pathname.includes('admin.html')) loadAdminPanel();
                        if (window.location.pathname.includes('activity.html')) loadActivity();
                        if (window.location.pathname.includes('suggest.html')) setTimeout(loadSuggestions, 500);
                    });
                }
            })
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const passwordInput = document.getElementById('authPassword');
    if (passwordInput) { passwordInput.addEventListener('input', checkPasswordOnType); }
    const nameInput = document.getElementById('authName');
    if (nameInput) { nameInput.addEventListener('input', checkUsernameOnType); }
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Enter') { const modal = document.getElementById('authModal'); if (modal && modal.style.display === 'flex') { handleAuth(); } }
    ...
    if (titleInput && titleCount) {
        ...
        titleInput.addEventListener('focus', function() { if (this.value.includes('@')) { this.value = ''; titleCount.textContent = '0'; } });
    }
    if (textInput && textCount) {
        textInput.addEventListener('input', function() { textCount.textContent = this.value.length; });
    }
    const commentText = document.getElementById('commentText');
    const commentCount = document.getElementById('commentCount');
    if (commentText && commentCount) {
        commentText.addEventListener('input', function() { commentCount.textContent = this.value.length; });
    }
    const searchInput = document.getElementById('searchInput');
    if (searchInput) {
        searchInput.addEventListener('keydown', function(e) { if (e.key === 'Enter') { searchStories(); } });
    }
    populateCountryDatalist();
    if (window.location.pathname.includes('profile.html')) loadProfile();
    if (window.location.pathname.includes('story.html')) loadStory();
    if (window.location.pathname.includes('admin.html')) loadAdminPanel();
    if (window.location.pathname.includes('activity.html')) loadActivity();
    if (window.location.pathname.includes('suggest.html')) setTimeout(loadSuggestions, 500);
