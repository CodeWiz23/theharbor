...
function handleAuth() {
    ...
    if (!emailInput || !passwordInput || !error || !submitBtn) {
...
function loadStories() {
    ...
    if (!canReadCategory(currentCategory)) {
        const note = getCategoryRestrictionNote(currentCategory) || 'You do not have permission to view this section.';
        ...
        container.innerHTML = `
            <div class="empty-state" style="padding:40px 20px;background:#f5d6b3;border-radius:16px;border-left:4px solid #c47a5a;">
                ...
                <p style="color:#2d3a3a;line-height:1.8;">${note}</p>
...
// ============================================
// SUBMIT STORY (Global function for submit.html)
// ============================================
...
function submitStory() {
    ...
    if (!titleInput || !textInput || !categorySelect || !error) {
    ...
    const gender = currentUserData.gender;
    if (category === 'men' && gender !== '🧔 Man') {
        error.textContent = '⚠️ Men\'s Harbor is for men only. Please select a different category.';
        return;
    }
    if (category === 'women' && gender !== '👩 Woman') {
        error.textContent = '⚠️ Women\'s Harbor is for women only. Please select a different category.';
        return;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- CHARACTER COUNTERS (Submit Page) ---
    const titleInput = document.getElementById('storyTitle');
    const textInput = document.getElementById('storyText');
    ...
    // --- CATEGORY RESTRICTION CHECK (Submit Page) ---
    const categorySelect = document.getElementById('storyCategory');
    if (categorySelect) {
        categorySelect.addEventListener('change', function() {
            // This will be handled by the function in submit.html
            if (typeof checkCategoryRestriction === 'function') {
